...
def extract_ts_features(x, ext_tsf_params=__default_ext_tsf_params, version=0):
    ...
    if _is_group_enabled("count_above_tb_"):
        ...
        if _is_enabled('count_above_tb_start_p'):
            f_list.append(count_above_zero(x_sub_tb_val_sign[0])/x.shape[1])
        if _is_enabled('count_above_tb_25p'):
            f_list.append(count_above_zero(x_sub_tb_val_sign[1]))
        if _is_enabled('count_above_tb_25p_p'):
            f_list.append(count_above_zero(x_sub_tb_val_sign[1])/x.shape[1])
    ...
    if _is_enabled('number_crossing_25p_p'):
        number_crossing_25p = tf.math.count_nonzero(diff(tf.sign(x - tf.expand_dims(quantiles[:,:,0], 1))), 1)
        f_list.append(number_crossing_25p/x.shape[1]*2)
    ...
    if _is_enabled('number_crossing_75p_p'):
        number_crossing_75p = tf.math.count_nonzero(diff(tf.sign(x - tf.expand_dims(quantiles[:,:,2], 1))), 1)
        f_list.append(number_crossing_75p/x.shape[1]*2)
    ...
    if _is_group_enabled("number_crossing_tb_"):
        ...
        if _is_enabled('number_crossing_tb_start'):
            f_list.append(tf.math.count_nonzero(diff(x_sub_tb_val_sign[0]), 1))
        if _is_enabled('number_crossing_tb_25p'):
            f_list.append(tf.math.count_nonzero(diff(x_sub_tb_val_sign[1]), 1))
        if _is_enabled('number_crossing_tb_median'):
            f_list.append(tf.math.count_nonzero(diff(x_sub_tb_val_sign[2]), 1))
        if _is_enabled('number_crossing_75p'):
            f_list.append(tf.math.count_nonzero(diff(x_sub_tb_val_sign[3]), 1))
        if _is_enabled('number_crossing_tb_end'):
            f_list.append(tf.math.count_nonzero(diff(x_sub_tb_val_sign[4]), 1))
    ######################################
    ...
    if _is_group_enabled('fft_'):
        ...
        if _is_enabled('fft_amp_zero_hz'):
            f_list.append(tf.expand_dims(fft_amp[:,:,0],2)) # fft_amp
    ...
    if _is_enabled('autocorralation'):
        ...
        if not _is_group_enabled('autocorralation_'):
            ...
            if _is_enabled('autocorralation_ratio_of_max'):
                f_list.append(tf.math.divide_no_nan(autocrrs, tf.math.reduce_max(tf.abs(autocrrs), axis=2, keepdims=True)))
    ...
    f_list2 = []
    for f in f_list:
        if f.dtype != tf.keras.mixed_precision.global_policy().compute_dtype:
...
class BlockedTsfMixer(AbstructLayer):
    ...
    def build(self, input_shape):
        ...
        self.bf_wise_mixer_ax_w = gen_mixer() if self.ax_weight_depth > 0 else None
        self.bf_wise_mixer_tsf_w = gen_mixer() if self.tsf_weight_depth > 0 else None
    ...
    def call(self, inputs):
        ...
        if _ax_w is not None:
            x = x * _ax_w
            x = x / tf.where(_ax_w == 0, 1., _ax_w)
        if _tsf_w is not None:
            x = x * _tsf_w
            x = x / tf.where(_tsf_w == 0, 1., _tsf_w)
